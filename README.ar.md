...
<details>
<summary><b>يقول macOS: «تعذّر على Apple التحقق…» أو «تالف»</b></summary>

<br>

النسخ غير موقّعة بشهادة Apple Developer ID. نزّل ملف `.pkg`: عندما يرفضه macOS في المرة الأولى اضغط **«تم»**، ثم افتح **«إعدادات النظام» ← «الخصوصية والأمن»**، وانزل إلى قسم «الأمن» واضغط **«فتح على أي حال»** (في macOS 13/14 يكفي النقر بالزر الأيمن على `.pkg` ← «فتح»). التطبيق المثبّت يعمل بعدها دون أي تحذير.

رسالة «تالف ولا يمكن فتحه» تظهر لتطبيق أُخذ من `.dmg` في الإصدارات القديمة: المتصفح يضع عليه علامة الحجر الصحي، وGatekeeper لا يسمح بتجاوز الرفض. ثبّت `.pkg` أو أزل العلامة يدويًا: `xattr -cr "/Applications/Aurora VPN.app"`
