...
/// The last field is the horizontal subpixel phase (0..SUBPIXEL_PHASES).
type GlyphCacheKey = (u64, u32, PhysicalLength, core::num::NonZeroU16, u64, u8);

/// На сколько долей пикселя делится горизонтальная позиция глифа.
pub const SUBPIXEL_PHASES: u8 = 4;

/// Показатель гамма-кривой, через которую пропускается покрытие глифа.
///
/// Апстрим смешивает покрытие линейно в sRGB, и светлый текст на тёмном фоне
/// из-за этого выходит жиже, чем его рисует DirectWrite: штрих шириной ~1.1px
/// размазывается по двум пикселям, и ни один не набирает полного цвета.
/// DirectWrite и Skia правят это гамма-коррекцией маски (у Skia — SkMaskGamma).
/// 0.70 подобрано по эталону: количество «чернил» строки сходится с WebView2 в
/// пределах 2.5% на 11.5, 12.5 и 25 px.
///
/// Кривая рассчитана на светлый текст по тёмному фону, то есть на тему
/// приложения. Для светлой темы (тёмный текст по светлому) коррекция нужна
/// обратная, показателем больше единицы, и её надо так же подобрать по эталону.
const COVERAGE_GAMMA: f32 = 0.70;

/// Таблица гамма-коррекции покрытия: 256 значений считаются один раз.
fn gamma_lut() -> &'static [u8; 256] {
    ...
    LUT.get_or_init(|| {
        ...
        for (i, v) in lut.iter_mut().enumerate() {
            *v = (255.0 * (i as f32 / 255.0).powf(COVERAGE_GAMMA)).round() as u8;
        }
        lut
    })
}
...
impl VectorFont {
    ...
    pub fn render_vector_glyph(
        ...
        self.render_vector_glyph_at(glyph_id, slint_context, 0)
    }

    pub fn render_vector_glyph_at(
        &self,
        glyph_id: core::num::NonZeroU16,
        slint_context: &i_slint_core::SlintContext,
        phase: u8,
        ...
        GLYPH_CACHE.with(|cache| {
            ...
            let glyph = {
                ...
                let mut scaler = ctx
                    ...
                    // Хинтинг: у Segoe UI Variable инструкции в шрифте есть
                    // (fpgm/prep/cvt), и с ними штрихи садятся на пиксельную
                    // сетку вместо того, чтобы расплываться по двум пикселям.
                    .hint(true)
                    .build();
