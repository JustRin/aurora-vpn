...
impl<T: ProcessScene> sharedparley::GlyphRenderer for SceneBuilder<'_, T> {
    ...
    fn draw_glyph_run(
        ...
        for positioned_glyph in glyphs_it {
            // Горизонтальная позиция приходит от parley дробной, а класть глиф
            // можно только на целый пиксель. Отбрасывать дробь — значит гулять
            // на ±1px по каждой букве: просветы между ними становятся рваными.
            // Вместо этого дробь уходит в фазу растеризации: сам глиф рисуется
            // сдвинутым на четверть пикселя, а в кэше под каждую фазу свой
            // рисунок.
            let x_total = positioned_glyph.x + global_offset.x as f32;
            let mut x_floor = x_total.floor();
            let mut phase =
                ((x_total - x_floor) * fonts::vectorfont::SUBPIXEL_PHASES as f32).round() as u8;
            if phase >= fonts::vectorfont::SUBPIXEL_PHASES {
                phase = 0;
                x_floor += 1.;
            }
            let x_floor = x_floor as i16;

            let Some(glyph) = std::num::NonZero::new(positioned_glyph.id as u16)
                .and_then(|id| font.render_vector_glyph_at(id, slint_context, phase))
            else {
                continue;
            };

            let glyph_offset: euclid::Vector2D<i16, PhysicalPx> = euclid::Vector2D::new(
                x_floor - global_offset.x,
                (euclid::Length::new(positioned_glyph.y) + y_offset).get() as i16,
            );
