...
/// `via` — локальный вход работающего ядра, если оно поднято.
///
/// Клиентский API Cloudflare на части сетей просто закрыт — на этой, например:
/// TCP-соединение принимают локально за шесть миллисекунд, а TLS не встаёт,
/// хотя сам cloudflare.com открывается. Отправленная напрямую, регистрация там
/// не проходит вовсе, и WARP нельзя даже включить. Поэтому, когда туннель уже
/// работает, запрос идёт через него — той же дорогой, что и трафик
/// пользователя.
fn client(via: Option<&str>) -> Result<reqwest::Client> {
    let builder = crate::net::http_builder()
        .timeout(Duration::from_secs(30))
        .user_agent(AGENT);
    let builder = match via {
        Some(url) => builder.proxy(reqwest::Proxy::all(url)?),
        // Иначе — мимо системного прокси: он может указывать на нас же, и
        // регистрация во время подъёма туннеля заклинила бы сама себя. То же
        // соображение, что и в обновлении подписок.
...
pub async fn register(via: Option<&str>) -> Result<WarpAccount> {
...
mod tests {
    ...
    async fn registers_a_usable_account_against_the_live_api() {
        let account = register(None).await.expect("регистрация должна пройти");
