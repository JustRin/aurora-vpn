...
mod imp {
    ...
    /// Задача есть и заведена именно на это приложение.
    ///
    /// Проверка та же, что перед передачей запуска: чужую задачу мы всё равно
    /// не станем запускать, значит и считать её включённым автозапуском нельзя.
    /// Иначе получается то, на что жалуются: галочка стоит, а приложение
    /// стартует без прав — потому что задача осталась от прежней установки и
    /// поднимает другой файл.
    pub fn task_is_ours() -> bool {
        if !task_exists() {
            return false;
        }
        match (task_command(), std::env::current_exe()) {
            (Some(registered), Ok(current)) => {
                registered.eq_ignore_ascii_case(&current.to_string_lossy())
            }
            _ => false,
        }
    }
