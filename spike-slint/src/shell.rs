...
pub fn on_close(ui: &AppWindow, handle: &AppHandle) {
    ...
    if to_tray {
        // Приложение уходит с глаз — камера предпросмотра не должна остаться
        // включённой: лампочка рядом с объективом горела бы, пока значок в
        // трее, и объяснить это было бы нечем.
        crate::bind::stop_camera(ui);
        // И из полоски окно возвращается на место: из трея оно должно выйти
        // обычным окном, а не узкой лентой поверх всего.
        crate::bind::leave_scan_mode(ui);
