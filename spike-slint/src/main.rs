...
struct Lang {
    ...
    notify_error: String,
    notify_orphan_title: String,
    notify_orphan_body: String,
    tray_show: String,
    tray_connect: String,
...
impl Lang {
    fn read(ui: &AppWindow) -> Self {
        ...
        Self {
            ...
            notify_orphan_title: s.get_notify_orphan_title().into(),
            notify_orphan_body: s.get_notify_orphan_body().into(),
...
///
/// Для китайского, японского, корейского и арабского Segoe Variable не годится:
/// своих знаков у него нет, а подстановка приводит первый попавшийся шрифт с
/// нужным покрытием — на пробном кадре упрощённый китайский так и остался с
/// дырами вместо 务, 连 и 迟, а арабский пришёл заметно мельче остального текста.
/// Поэтому у каждой письменности берётся её системный шрифт; заодно это снимает
/// ханьскую унификацию, из-за которой японский текст японским шрифтом и
/// китайским выглядит по-разному.
#[cfg(windows)]
fn ui_font(lang: i32) -> &'static str {
    let script = match LANGS.get((lang - 1).max(0) as usize) {
        Some(&"zh") => Some(("msyh.ttc", "Microsoft YaHei UI")),
        Some(&"ja") => Some(("YuGothR.ttc", "Yu Gothic UI")),
        Some(&"ko") => Some(("malgun.ttf", "Malgun Gothic")),
        // Вариативный Segoe арабского не знает, и подстановка приводила шрифт
        // с заметно более мелким очком. У статического Segoe UI арабский есть
        // — это и есть системный шрифт арабского интерфейса Windows.
        Some(&"ar") => Some(("segoeui.ttf", "Segoe UI")),
        _ => None,
    };
    if let Some((file, family)) = script {
