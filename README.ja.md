...
<details>
<summary><b>macOS に「Apple は…を検証できません」または「壊れている」と表示される</b></summary>

<br>

ビルドは Apple Developer ID で署名されていません。`.pkg` をダウンロードしてください。初回に macOS が拒否したら**［完了］**を押し、**［システム設定］→［プライバシーとセキュリティ］**を開いて「セキュリティ」までスクロールし、**［このまま開く］**を押します（macOS 13/14 では `.pkg` を右クリック →［開く］でも同じです）。インストールされたアプリは警告なしに起動します。

「壊れているため開けません」は、旧バージョンの `.dmg` から取り出したアプリです。ブラウザが検疫フラグを付け、Gatekeeper はこれを回避させてくれません。`.pkg` を使うか、フラグを手動で外してください: `xattr -cr "/Applications/Aurora VPN.app"`
