...
<details>
<summary><b>macOS пишет «Не удалось проверить…» или «повреждено»</b></summary>

<br>

Сборки не подписаны сертификатом Apple Developer ID. Скачайте `.pkg`: когда macOS откажет в первый раз, нажмите **«Готово»**, откройте **«Системные настройки» → «Конфиденциальность и безопасность»**, пролистайте до раздела «Безопасность» и нажмите **«Всё равно открыть»** (на macOS 13/14 то же делает ПКМ по `.pkg` → «Открыть»). Установленное приложение запускается без предупреждений.

«Повреждено, и его не удаётся открыть» — это приложение из `.dmg` старых версий: браузер ставит на него флаг карантина, и Gatekeeper обойти отказ не даёт. Поставьте `.pkg` или снимите флаг вручную: `xattr -cr "/Applications/Aurora VPN.app"`.
...
| **Балансировка** | с резервом, самый быстрый с порогом или по кругу — решает приложение, а не urltest ядра, так что серверы с одинаковой задержкой не скачут |
...
| **Android** | виджеты на рабочий стол — кнопка, статус со скоростью, полная карточка с трафиком и временем сеанса — и плитка в шторке; подключение без открытия приложения |
