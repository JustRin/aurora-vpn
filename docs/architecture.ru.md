...
Общее для автоматических стратегий: текущий узел проверяется каждые 20 секунд
(после осечки — через пять), остальные — обходом раз в несколько минут, кусками
по восемь, чтобы обход не задерживал проверку текущего. Кандидат с устаревшим
замером перед переходом перепроверяется.
