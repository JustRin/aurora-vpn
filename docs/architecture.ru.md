...
Смена стратегии или её параметров ядро не перезапускает: меняется только
автомат. Выбор сервера руками при «самом быстром» и «по кругу» выключает
автоматику — иначе балансировщик увёл бы трафик обратно через минуту; при
«с резервом» выбранный узел просто становится основным. Узлы vmess+ws, не
унесённые в Xray, автомат не меряет и не выбирает: неудачный дозвон до такого
узла роняет ядро (см. auto-группу в `core/config.rs`); основным по воле
пользователя такой узел быть может, но пока трафик идёт мимо, его не трогают.
