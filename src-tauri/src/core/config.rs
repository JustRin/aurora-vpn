...
pub fn build(input: &BuildInput) -> Result<BuiltConfig> {
    ...
    //
    // Узлы WARP держатся вне группы по своей причине: urltest — это дозвоны
    // до всех её участников каждые три минуты, а Cloudflare на один ключ
    // держит одну живую сессию, и такие дозвоны выбивают рабочий туннель.
