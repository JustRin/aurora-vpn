...
async fn await_control_plane(app: &AppHandle, api: &ClashApi) -> std::result::Result<(), String> {
    ...
    loop {
        ...
        if Instant::now() >= deadline {
            // Слой WARP — единственное, что умеет задержать сам старт: узел
            // выходит на связь лениво, а endpoint поднимает туннель ещё во
            // время загрузки, и через `detour` это значит дозвон до сервера.
            // До недоступного сервера дозвон высиживает свой таймаут (15 с на
            // Windows, дольше на Linux), и без этой подсказки причиной выглядит
            // ядро, а не мёртвый сервер под слоем.
            let layered = app.state::<AppState>().settings.read().warp_over_proxy;
            return Err(if layered {
                "ядро запущено, но панель управления не ответила за 30 с. Включена \
                 дополнительная защита через WARP: её туннель поднимается через выбранный \
                 сервер, и недоступный сервер задерживает запуск. Проверьте сервер или \
                 выключите WARP"
                    .into()
            } else {
